...
export default {
  ...
  methods: {
    ...
    MatchingTools: function () {
      ...
      var c_count = 0
      var countrow = 0
      // var countrowOposit = 0
      if (this.outputWordsLeft.countrow >= this.outputWordsRight.countrow) {
        countrow = this.outputWordsRight.countrow
        // countrowOposit = this.outputWordsLeft.countrow
      } else {
        countrow = this.outputWordsLeft.countrow
        // countrowOposit = this.outputWordsRight.countrow
      }
      console.log('countrow', countrow,rightdatafilter.length)

      // Loop ใหญ่
      for (let i = 0; i < countrow; i++) {
        let data1 = leftdata[c_count].column1.trim().toString()
        // Loop สอง เพื่อไล่หาที่ละอัน
        console.log('1st loop',i);
        for (let c = 0; c < rightdatafilter.lenght ; c++) {
